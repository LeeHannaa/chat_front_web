  ...
  <main>
    <div class="main">
      ...
      <div v-if="userStore.userList.length > 0">
        <div class="user" v-for="user in userStore.userList" :key="user.id" style="cursor: pointer">
          <h3>{{ user.name }}</h3>
          <button
            :class="{ groupBT: true, active: selectedUserIds.includes(user.id) }"
      ...
      <button class="groupCreateBT" @click="handleGroupCreateClick()">방 만들기</button>
    </div>
  </main>
</template>
